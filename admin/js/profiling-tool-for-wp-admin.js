	 ...
	 function addListeners() {
		...
		$('#saveProfile').on( "click", function( event ) {
		 ...
		 $('#run_quick_tests').on( "click", function() {
            ...
            $('#run_quick_tests').html('<span style="font-size: 16px;">' + lang.Scanning + '...<div class="loader"></div></span>');
            ...
            var myInterval = setInterval(function() 
                ...
                if(current_element == elements_all_plugins.length-1)
                {
                    ...
                    $('#run_quick_tests').html(lang.Start);
    ...
    jQuery.ajax({
        ...
        success: function (result) {
            ...
            if (startStringQuery >= 0)
            {
                ...
                queries_number+= ', ' + lang.Query_DB + ': ' + queriesNumber;
            }
            ...
            if (startStringPMemory >= 0)
            {
                ...
                queries_number+= ', ' + lang.Memory_peak + ': ' + pMemory+' MB';
            }
        ...
        resultSpan.html(lang.Request_time + ': '+totalTime+'ms'+queries_number);
...
function generate_report() {
	...
	reportText+= '<h3>' + lang.Results_from_test + '</h3>';
	reportText+= lang.You_will_get_better_result_if_disactive_plugins + ':<br /><br />';
...
function makePageTableHtml(url, time) {
	...
	result += '<td><b>' + lang.Page + '</b></td>';
	result += '<td><b>' + lang.Load_time + ' (s):</b></td>';
	result += '</tr>';
...
function saveProfile ( data ) {
	...
	var nombre = "profile_" + fecha.toString();
...
function makeTableHTML( myArray ) {
	...
	result += "<thead>";
	result += "<tr>";
	result += "<td><b>" + lang.Time + "</b></td>";
	result += "<td style='text-align:center'><b>" + lang.Name + "</b></td>";
	result += "<td style='text-align:center'><b>" + lang.Result + "</b></td>";
	result += "</tr>";
	result += "</thead>";
...
function makeGrafics(array) {
	...
	new Chart(ctx, {
		...
		title: {
			display: true,
			text: lang.Plugin_execution_time_in_mileseconds
		},
		options: {
			scales: {
				y: {
